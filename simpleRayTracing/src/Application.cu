...
//}
//
//__forceinline __host__ float3 rayEnvironmentIntersection(Trace::Ray& ray, Trace::Environment* env) {
//
//    int nBounces = 1;
...
//}
//
//__forceinline __host__ bool rayEnvironmentIntersectionTEST(Trace::Ray& ray, Trace::Environment* env) {
//
//    int nBounces = 1;
//    float3 attenuation = make_float3(1.0f, 1.0f, 1.0f);
//    float3 background = make_float3(0.0f, 0.0f, 0.0f);
...
//
//
//__forceinline __host__ float3 computeBarycenter(Trace::Primitive* primitive, int objID) {
//
//    float3 A = primitive->vertex[primitive->face[objID].x];
//    float3 B = primitive->vertex[primitive->face[objID].y];
//    float3 C = primitive->vertex[primitive->face[objID].z];
//    return (A + B + C) / 3.0f;
//}
//
//void execute_k(
...
//    for (int k = 0; k < env->primitive->N; k++) {
//
//        // Compute barycenter
//        float3 target = computeBarycenter(env->primitive, k);
//
//        // Project ray
//        float3 direction = normalize(target - camera.position);
//        Trace::Ray ray(camera.position, direction);
//
//        bool hitSomething = rayEnvironmentIntersectionTEST(ray, env);
//
//        if (!hitSomething) {
//            int shit = 1 + 1;
//        }
