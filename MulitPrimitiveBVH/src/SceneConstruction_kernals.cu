...
void modifiedBunny(std::vector<float3>& vertices, std::vector<int3>& indices) {
    ...
    for (int i = 0; i < N_vtx; i++) {
        min = fminf(min, vertices[i]);
        max = fmaxf(max, vertices[i]);
    }

    // Modify bunny coordinates so that it's centered and scaled
    max -= min;
...
__global__ void initBunny(Trace::PrimitiveArray** env, Trace::Primitive** objects, Trace::Material** materials, float3* v, int3* f, int nFaces)
{
    if (threadIdx.x == 0 && blockIdx.x == 0) {

        materials[0] = new Trace::Lambertian(make_float3(0.7f, 0.3f, 0.3f)); // pink
        materials[1] = new Trace::Reflective(make_float3(0.1, 0.1, 0.1), 0.05f); // dark
        materials[2] = new Trace::Reflective(make_float3(0.3f, 0.8f, 0.4f), 0.0f); // ugly

        int* matID = new int[nFaces];
        for (int k = 0; k < nFaces - 2; k++){
            matID[k] = 0;
        }
        matID[nFaces - 2] = 1;
        matID[nFaces - 1] = 2;
        ...
        Trace::Primitive* triangle = new Trace::Triangle(v, f, nFaces, matID);

        //int idx = 0;
        //while (idx < nFaces - 2) {
        //    Trace::Primitive* triangle = Trace::Primitive::Triangle(v[f[idx].x], v[f[idx].y], v[f[idx].z], pink);
        //    objects[idx] = triangle;
        //    idx++;
        //}
        //objects[idx] = Trace::Primitive::Triangle(v[f[idx].x], v[f[idx].y], v[f[idx].z], dark);
        //idx++;
        //objects[idx] = Trace::Primitive::Triangle(v[f[idx].x], v[f[idx].y], v[f[idx].z], ugly);
        //idx++;


        *env = new Trace::PrimitiveArray(objects, materials, nFaces);
...
extern "C" void setUpPrimitiveArray(SceneType sceneType, Trace::Pipeline & pipeline) {
    ...
    Trace::Primitive** d_primitives;
    Trace::Material** d_materials;
    ...
    switch (sceneType) {
        case (SceneType::CORNELL_BOX):
        {
            ...
            //// Allocate memory
            //d_primitiveArray = Trace::PrimitiveArray::allocate();
            //d_primitives = Trace::Primitive::allocate(nPrimitives);
            //initCornellBox << <1, 1 >> > (d_primitiveArray, d_primitives);
            //break;
        ...
        case (SceneType::BUNNY):
        {
            ...
            // Camera
            origin = make_float3(0.0f, 2.0f, 1.0f);
            ...
            modifiedBunny(vertices, indices);
            int nVertices = vertices.size();
            nPrimitives = indices.size();


            // Ammend vertices and indices to add more
            ...
            indices.push_back(make_int3(nVertices + 0, nVertices + 1, nVertices + 2));
            nVertices += 3;
            nPrimitives += 1;
            ...
            float3* d_vertices;
            int3* d_indices;
            cudaMalloc((void**) &d_vertices, nVertices * sizeof(float3));
            cudaMalloc((void**) &d_indices, nPrimitives * sizeof(int3));

            // Copy vertices and indices from host to device
            cudaMemcpy(d_vertices, vertices.data(), nVertices * sizeof(float3), cudaMemcpyHostToDevice);
            cudaMemcpy(d_indices, indices.data(), nPrimitives * sizeof(int3), cudaMemcpyHostToDevice);


            // Allocate memory
            d_primitiveArray = Trace::PrimitiveArray::allocate();
            d_primitives = Trace::Primitive::allocate(nPrimitives);
            d_materials = Trace::Material::allocate(3);
            initBunny << <1, 1 >> > (d_primitiveArray, d_primitives, d_materials, d_vertices, d_indices, nPrimitives);
            break;
        }
    ...
    // Scene Camera
    Trace::Camera* d_camera = new Trace::Camera(origin, target, vFOV, computeAspectRatio(pipeline));

    pipeline.camera = *d_camera;
    pipeline.d_environment = d_primitiveArray;
...
//__global__ void singleSphere(Trace::PrimitiveArray** env, Trace::Primitive** objects)
//{
//    if (threadIdx.x == 0 && blockIdx.x == 0) {
//
//        Trace::Material* dark = new Trace::Reflective(make_float3(0.1, 0.1, 0.1), 0.05f);
//        objects[0] = Trace::Primitive::Sphere(make_float3(0.0f, 1.0f, 0.0f), 1.0f, dark);
//
//
//        *env = new Trace::PrimitiveArray(objects, 1);
//    }
//}
//
...
//        objects[0] = Trace::Primitive::Triangle(
//            make_float3(-50.0f, 0.0f, -50.0f),
//            make_float3(50.0f, 0.0f, -50.0f),
//            make_float3(-50.0f, 0.0f, 50.0f), dark);
//
//        objects[1] = Trace::Primitive::Triangle(
//            make_float3(50.0f, 0.0f, -50.0f),
//            make_float3(-50.0f, 0.0f, 50.0f),
//            make_float3(50.0f, 0.0f, 50.0f), ugly);
//
//        objects[2] = Trace::Primitive::Sphere(make_float3(2.0f, 1.0f, 2.0f), 1.0f, dark);
//        objects[3] = Trace::Primitive::Sphere(make_float3(-2.0f, 1.0f, 1.0f), 1.0f, pink);
//        objects[4] = Trace::Primitive::Sphere(make_float3(0.25f, 0.25f, 0.25f), 0.25f, grey);
//
...
//        objects[2] = Trace::Primitive::Sphere(centers[0], 1.0f, mat1);
//
//        Trace::Material* mat2 = new Trace::Lambertian(make_float3(0.4f, 0.2f, 0.1f));
//        objects[3] = Trace::Primitive::Sphere(centers[1], 1.0f, mat2);
//
//        Trace::Material* mat3 = new Trace::Reflective(make_float3(0.7f, 0.6f, 0.5f), 0.0f);
//        objects[4] = Trace::Primitive::Sphere(centers[2], 1.0f, mat3);
//
//
//        int objCount = 5;
//        const int spread = 11;
...
//                    objects[objCount] = Trace::Primitive::Sphere(center, 0.2f, mat);
//                    objCount++;
//
//                }
//                else {
